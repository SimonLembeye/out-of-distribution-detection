...
class WideLayer(nn.Module):
    def __init__(self, num_channels, num_out, dropout, stride):
        ...
        self.conv1 = nn.Conv2d(
            num_channels, num_out, 3, padding=1, bias=False, stride=stride
        )

        self.convx = nn.Conv2d(
            num_channels, num_out, 1, padding=0, bias=False, stride=stride
        )
...
class WideResNet(nn.Module):
    def __init__(self, num_classes, depth=28, width=10, dropout=0.3):
